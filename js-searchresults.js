function loadDirectionsTab(destination) {
    ...
    .then(data => {
        ...
        // Select and initialize elements
        ...
        document.getElementById('returnButton').addEventListener('click', function() {
            // Navigate back to the main HTML file
            window.location.href = 'html-main.html'; // Replace 'main.html' with the path to your main HTML file
        });
       
        searchForm.addEventListener('submit', function(event) {
            event.preventDefault();
            let result = calculateResult(from.value.toLowerCase(), to.value.toLowerCase(), from, to); // Pass 'from' and 'to' as parameters
            displayResult(result, categories, from, to); // Pass 'from' and 'to' to displayResult
        });
        // Hide the search overlay
        document.getElementById('searchOverlay').style.display = 'none';
        // Dynamically insert link to external CSS file
        const linkElement = document.createElement('link');
        linkElement.rel = 'stylesheet';
        linkElement.href = 'css-searchresults.css'; //  ✦ . 　⁺ 　 . ───────── ౨ LAGAYAN NG LINK TAG ৎ ─────────  . 　⁺ 　 . ✦   */
